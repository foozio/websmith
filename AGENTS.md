...
## Testing Guidelines
Use Vitest with Testing Library helpers and `@testing-library/jest-dom` assertions. Add at least one behavior test for every new component path. For tokens or theming changes, include snapshot/rendering coverage. Keep `npm run test` green before requesting review and note untested areas in the PR body.
